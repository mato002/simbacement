...
@section('content')
    ...
    >
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            ...
            <div class="space-y-5">
                <div id="product-images" class="space-y-5">
                    ...
                    <div
                        ...
                        x-data="{
                            ...
                            onChange(event) {
                                ...
                                this.preview = file && file.type.startsWith('image/')
                    ...
                    >
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-steel">Main photo used on catalogue cards and product page.</p>
                            </div>
                            @if ($product->exists && $primary?->media)
                                <span class="admin-badge admin-badge-success" x-show="!preview">Set</span>
                                <span class="admin-badge admin-badge-success" x-show="preview" x-cloak>New</span>
                            @else
                                <span class="admin-badge admin-badge-muted" x-show="!preview">Missing</span>
                                <span class="admin-badge admin-badge-success" x-show="preview" x-cloak>Ready</span>
                            @endif
                        </div>

                        {{-- Live preview of newly selected file --}}
                        <div x-show="preview" x-cloak class="mt-4 overflow-hidden border border-line bg-mist">
                            <img :src="preview" alt="Selected primary image" class="aspect-square w-full object-cover">
                            <p class="border-t border-line bg-white px-3 py-2 text-xs font-semibold text-ink">Preview · save to keep this image</p>
                        </div>
                        ...
                        <div x-show="!preview">
                            ...
                            @else
                                <div class="mt-4 flex aspect-square w-full items-center justify-center border border-dashed border-line bg-mist text-center text-sm text-steel">
                                    No primary image yet — choose a file below
                                </div>
                            @endif
                        </div>

                        <label class="mt-4 mb-1.5 block text-sm font-semibold">
                            {{ $primary?->media ? 'Replace primary image' : 'Upload primary image' }}
                        </label>
                        <input
                            type="file"
                            name="primary_image"
                            ...
                            @change="onChange"
                            class="w-full border border-line bg-mist px-3 py-2 text-sm"
                        >
                        <p class="mt-1 text-xs text-steel">JPG, PNG or WebP · max 5MB · preview appears above as soon as you choose a file</p>
                    ...
                    >
                        <h2 class="font-display text-2xl font-bold uppercase tracking-wide">Gallery images</h2>
                        <p class="mt-1 text-xs text-steel">Extra photos for the product detail page. You can add several at once.</p>
                        ...
                        @if ($product->exists && $product->images->where('is_primary', false)->isNotEmpty())
                            <div class="mt-4 grid grid-cols-2 gap-3">
                                @foreach ($product->images->where('is_primary', false) as $image)
                                    <label class="relative block cursor-pointer overflow-hidden border border-line bg-mist">
                                        @if ($image->media)
                                            <img src="{{ $image->media->url() }}" alt="" class="aspect-square w-full object-cover">
                                        @endif
                                        <span class="absolute inset-x-0 bottom-0 bg-ink/85 px-2 py-1.5 text-[11px] font-semibold text-white">
                        ...
                        @elseif ($product->exists)
                            <div class="mt-4 border border-dashed border-line bg-mist px-4 py-6 text-center text-sm text-steel" x-show="previews.length === 0">
                                No gallery images yet
                            </div>
                        @endif

                        <div x-show="previews.length > 0" x-cloak class="mt-4">
                            <p class="mb-2 text-xs font-semibold uppercase tracking-wide text-steel">New selections</p>
                            <div class="grid grid-cols-2 gap-3">
                                <template x-for="(src, index) in previews" :key="src">
                                    <div class="overflow-hidden border border-line bg-mist">
                                        <img :src="src" :alt="'Gallery preview ' + (index + 1)" class="aspect-square w-full object-cover">
                                    </div>
                                </template>
                            </div>
                            <p class="mt-2 text-xs text-steel">Preview only · save the product to upload these images</p>
                        </div>

                        <label class="mt-4 mb-1.5 block text-sm font-semibold">Add gallery images</label>
                        <input
                            type="file"
                            name="gallery_images[]"
                            accept="image/jpeg,image/png,image/webp,image/gif"
                            ...
                            class="w-full border border-line bg-mist px-3 py-2 text-sm"
                        >
                        <p class="mt-1 text-xs text-steel">Select one or more images · max 5MB each</p>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            @if ($product->exists)
                <a href="#product-images" class="btn-dark">Manage images</a>
            @endif
            <a href="{{ route('admin.products.index') }}" class="inline-flex items-center justify-center gap-2 border border-line bg-white px-5 py-3 text-sm font-semibold tracking-wide text-ink uppercase transition hover:border-brand">Back to products</a>
        </div>
    </form>
